...
{
    QString dir = QFileDialog::getExistingDirectory(nullptr, "Select Loaded Containers route",  QDir::homePath(),QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks );
    ...
    for (const QString& fileName : fileList) {
        ...
        while (offset + 16 <= extractDataraw.size()) {  // 确保至少16字节
            ...
            if (dataStart >= dataEnd) {
                std::cerr << "Error: Invalid data range! dataStart: " << dataStart << ", dataEnd: " << dataEnd << std::endl;
                break;
            }

            // 提取数据
            std::vector<uint8_t> data(extractDataraw.begin() + dataStart, extractDataraw.begin() + dataEnd);
        ...
        // 逐块写入文件
        std::ofstream outFile(outputFilePath, std::ios::binary | std::ios::in | std::ios::out);
        if (!outFile) {
...
void bulk_decode::on_pushButton_5_clicked()
{
    loadSettings();
}
